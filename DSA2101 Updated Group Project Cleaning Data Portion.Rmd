...
### Non-Group stage
As the host country might have been eliminated in the group stages or along the non-group stage leading to the finals, the number of games played by the host would not necessarily equal to total possible matches they could have played. As such, we would need to find the number of distinct non-group stages (e.g. Semi-Finals, Finals) for each edition of the World Cup, with the assumption that each stage can only consists of one match per team. The assumption is fair given the standard non-group stage structure of the World Cup except for the 1950 edition, which we will tabulate separately.

Firstly, we filter out the year 1950 and group stages from `wcmatches_tidy`. Similar with the group stage, we group the data by year and country, then summarise using the n_distinct function on stage_status. This provides us the number of unique non-group stage stages of each world cup which equals to the total possible matches a country could have played.This data is stored as `wc_elimstage_matches`.

Secondly, for the 1950 World Cup edition, the non-group stage structure is similar to the group stage of other world cups. Therefore, we tabulate the data in a similar way as that used for the group stage matches. We filter for year 1950 from `wcmatches_tidy`, non-group stage matches whereby the home team is the host country. We group by year and country then summarise the total number of matches played by the host country which equals total possible matches. Lastly we bind the rows of this data with `wc_elimstage_matches` to get a complete dataset for the non-group stage total possible matches as `wc_elimstage_matches1`.

### Merging of Data
Lastly, we merge the `wc_groupstage_matches` with `wc_elimstage_matches1` using full join by year and country as some World Cup editions do not have a group stage. For the World Cups that do not have a group stage, we replaced the number of group stage matches played as 0. We then summed up the number of group stage and non-group stage matches to find the total possible matches for every world cup. We summarised the data by country to combine different world cups with the same host.
...
ggplot(data = combine_data, aes(x = proportion_win_away, y = proportion_win))+
  geom_point()+
  geom_text(aes(label = country), hjust= 0.5,vjust = -0.5,angle = -30,size = 3) +
  ylim(0,110)+
...
Through our visualisations, we aim to provide insights into a host country's performance.

For the first plot, we realise that the countries with the highest points have generally hosted the World Cup at least once (Brazil in 1950 and 2014, Germany in 1974 and 2006, Italy in 1934, Argentina in 1978) and have obtained higher total points than countries that have never hosted the World Cup before. [plot needs to be refined to say this...]
