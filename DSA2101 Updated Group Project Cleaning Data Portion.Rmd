...
2. Visualising the total points obtained on a heatmap.

### Part 1: Preparing the Data

As per FIFA convention, for each match, we award 3 points to the winning team, 0 points to the losing team and 1 point to both teams if it ends in a draw. There are certain exceptions to the point allocation for the countries:
...
We focused on rectifying the exceptions as mentioned above, and tabulate the points obtained by each country from the number of wins and draws for each country over time. We also want to ensure that all teams that participated in the World Cup at least once are included in the merged dataset, as some countries may have failed to obtain any points (e.g. China in 2002). This is for us to differentiate between the latter (represented by light yellow, `#ffffb2` in the heatmap) and countries that have never participated in the World Cup before (represented by `grey` in the heatmap).

Some former teams like the Soviet Union and Yugoslavia were renamed to their present-day countries in preparation for our heatmap using `map_data("world")` from the `maps` package (in Part 2).
...
We want to create a scatter plot to compare the proportion of games a country wins when they are the host of the World Cup against when they are not the host. We need to find 2 variables to plot the scatterplot:

1. Proportion of matches won by host country of every World Cup

2. Proportion of matches won by non-host countries

### Part 1: Tabulating the variables
Firstly, we tabulate the proportion won by the host country by finding the ratio of matches won to matches played by the host country every year. Secondly, we find the ratio of wins by non-host countries to their total matches played across all World Cups. A linear regression line will be plotted to display the general trend between the 2 variables. By plotting these 2 variables, we are able to visualize how being the host of the World Cup can affect the country's performance by whether the point of the countries are located below or above the linear regression line. This will allow us to predict more accurately the winner of World Cup matches depending on the host of the World Cup.

### Part 2: Visualisation of graph
...
ggplot(data = wc_win_prop_combine, aes(y= not_host_wins_prop,x = host_win))+
  geom_point()+
  geom_text_repel(aes(label = country), hjust = 0.05, vjust = -0.5,angle = 45,size = 2.5)+
  ylim(0,80)+
...
Through our visualisations, we aim to provide insights into the different factors that may affect a country's performance at the World Cup. Various factors include the continent of the country and whether the country has been a host in the World Cup. 
From our first visualisation (heatmap), we can observe that the most successful nations at the World Cup throughout its history are generally from Europe and South America. This is represented by the orange and red fill colours of the countries in Europe and South America (e.g. Brazil, Italy, Germany) where the total points obtained are higher. Countries in other continents (North America, Asia, Africa and Oceania) have lighter shades of orange and yellow, where the total points obtained are lower. A possible reason for the better performance in European and South American countries is that football might be the primary sport for these nations and more players from Europe and South America have been training hard in football for many years. Due to the long history of these countries in football, their skill level in football is generally higher and there is a higher probability that European and South American countries will perform better in the World Cup, leading to higher points. Another trend observed from the heatmap is that for South America and Europe, almost all nations have participated in the World Cup (have a certain fixed colour) but for the other continents, many nations have not participated in the World Cup (a lot of countries that are filled white). A possibility is that since Europe and South America have better teams, they deserve more berths and are allocated more slots to the confederation (Lopez, 2022).

From our second visualisation (scatterplot), we can observe that 

From our third visualisation (lineplot), we can observe that
