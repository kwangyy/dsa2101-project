...
ggplot(data = top_countries_joined, mapping = aes(x = long, y = lat, group = group)) +
  ...
  theme(axis.text = element_blank(),
        ...
        borders(database = "world", regions = ".", fill = NA, colour = "black", size = 0.5) + 
        theme_map()
...
draw_data <- wcmatches_tidy %>%
             filter(year >= 1990, win_outcome == "Draw", penalties == 0) %>%
...
combined_draw_data <- draw_data_home %>%
                      full_join(draw_data_away, by = c("year" = "year", "stage_status" = "stage_status", "home_team" = "away_team")) %>%
...
final_dataset <- winning_data %>%
                 full_join(combined_draw_data, by = c("year" = "year", "stage_status" = "stage_status", "winning_team" = "home_team")) %>%
                 mutate(win_points = replace_na(win_points, 0), 
                        draw_points = replace_na(draw_points, 0),
                        grand_total_points = win_points + draw_points) %>%
                 select(year, winning_team, grand_total_points) %>%
                 mutate(continent = case_when(
                   winning_team %in% c("Argentina", "Brazil", "Costa Rica", "Uruguay", "United States", "Colombia", "Mexico", "Croatia", "Morocco", "Jamaica", "Paraguay", "Ecuador", "Senegal", "Peru", "Greece", "Ivory Coast", "Chile", "Honduras", "Bolivia", "Trinidad and Tobago") ~ "Americas", 
                   winning_team %in% c("Austria", "Belgium", "England", "Italy", "Romania", "Scotland", "Spain", "West Germany", "Yugoslavia", "Czechoslovakia", "Germany", "Netherlands", "Norway", "Republic of Ireland", "Sweden", "Switzerland", "Denmark", "FR Yugoslavia", "France", "Serbia", "Poland", "Portugal", "Czech Republic", "Bosnia and Herzegovina", "Ukraine", "Slovenia", "Iceland") ~ "Europe", 
                   winning_team %in% c("Cameroon", "Czechoslovakia", "Soviet Union", "Bulgaria", "Nigeria", "Russia", "Saudi Arabia", "Iran", "Japan", "South Africa", "South Korea", "Australia", "Turkey", "Ghana", "Tunisia", "Algeria", "Slovakia", "New Zealand", "Angola", "Egypt") ~ "Rest Of World"
                 )) %>%
