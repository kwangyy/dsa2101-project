...
We want to create a line plot that determines whether the stage status of the matches will affect the goal differences of the matches. 
...
```
Next, for the `worldcups_tidy` dataset, we similarly group each observation by year and calculate the number of goals_scored for each year (for the host country) for all matches. Next, we rename the host_country column of the dataset to country for easier joining of datasets subsequently. Save this in a dataset called `viz_3_worldcups`.
...
Afterwards, we join thr `viz_3_wcmatches` and `viz_3_worldcups` dataset together by year and afterwards, calculate the proportion of goals that the host countries have scored during their runs. Save this in a dataset called `viz_3_data`. 
...
Lastly, we can perform `ggplot` to create a line plot for the proportion of goals that the host countries have scored during their runs for the respective years using geom_line. The years 1942 and 1946 are not included and we simply connect the 1938 proportion and the 1950 proportion together using a straight line. 
