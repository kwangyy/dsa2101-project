...
As our objective is to show the total points obtained across all World Cup matches for each country, we should first tabulate the all-time total points obtained for each country.

### Part 1: Preparing the Data

As per FIFA convention, for each match, we award 3 points to the winning team, 0 points to the losing team and 1 point to both teams if the match ends up in a draw.
...
We also included countries that were involved in the World Cup at least once, but did not score any points at all over the years.
...
Afterwards, we do a full join of `wcmatches_wins`, `wcmatches_draws` and `wcmatches_allcountries` to ensure that all teams are contained in the merged dataset. We use `full_join` because we want to include all countries that ever participated in the World Cup.

Certain entries in the merged dataset contain `NA` values as some countries are not found in either `wcmatches_draws` or `wcmatches_wins`. The `NA` values should be replaced by `0`. We compute the total points for each country by summing the draw points and win points.

Certain teams such as Soviet Union, Yugoslavia and Czechoslovakia were renamed to their present-day countries in order to merge with the `world_coordinates` dataset. We then stored it in a new dataset called `wcmatches_tabulated`.
...
We now proceed to put this information on the total points obtained on a heatmap. We will do this using the world map.

First, we obtain information on the coordinates of the countries by using the `map_data("world")`. We had to do some data cleaning of the names of the countries to match those in the `wcmatches_tabulated` dataset. For example, we noticed that certain subregions in the dataset are not properly separated into the respective regions (eg. Scotland, Great Britain, Isle of Wight and Wales are all under UK in the region column of the dataset). However, these countries should be differentiated by region, as per FIFA's convention.
...
Next, we join the `world_coordinates` dataset with the `wcmatches_tabulated` dataset by the name of the region. All the country names in the `wcmatches_tabulated` exist in the region column of the `world_coordinates` dataset.
...
Lastly, we use `ggplot` to generate the heatmap that showcase the total number of points various countries obtained from the World Cup matches over the years. Countries that are not involved in the World Cup have a score of `NA` in the `top_countries_joined` dataset and hence, are not shaded in the heatmap.
...
```{r Line Plot}
stage_levels <- c("Group Stage", "Knockout Stages", "Final Stages")
knockout_stages <- c("Quarterfinals", "Round of 16", "Semifinals")
final_stages <- c("Final", "Third place")


diff_avg_plot <- wcmatches_tidy %>% 
  # We mutate this first so that the code is a lot more readable
  mutate(stage_status = case_when(stage_status %in% knockout_stages ~ "Knockout Stages",
                                  stage_status %in% final_stages ~ "Final Stages",
                                  stage_status == "Group Stage" ~ "Group Stage")
                                  ) %>%
  # We then mutate it according to difference and stage_level
  mutate(stage_status = factor(stage_status, stage_levels)) %>%
  group_by(year, stage_status) %>%
  summarize(diff_avg = mean(score_difference)) 
