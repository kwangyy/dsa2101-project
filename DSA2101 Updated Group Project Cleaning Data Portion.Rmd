...
Note: `after_extra_time` does not affect the victory/loss of the teams. 
...
world_coordinates = map_data("world") %>%
  mutate(region = case_when(
    ...
    .default = region))
# I need to think of a way to resolve England and Scotland
head(world_coordinates, 10)

# sorry I had to copy over the wcmatches_tabulated data here to tabulate every single country's result.
# curr_team refers to the modern day country (e.g. west germany and east germany is modern day germany)
wcmatches_total_pts =
  ...
  mutate(curr_team =
           case_when(
             ...
             team == "FR Yugoslavia" ~ "Serbia",
             team == "Czechoslovakia" ~ "Czech Republic",
             .default = team)) %>%
  group_by(curr_team) %>%
  summarise(total_points = sum(final_score))

top_countries_joined = world_coordinates %>%
  left_join(wcmatches_total_pts, by = c("region" = "curr_team"))
