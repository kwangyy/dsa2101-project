...
We want to see if geographical factors affect the performance of the World Cup. Therefore, we manipulated the data to tabulate the all-time total points for each country, before showcasing the total points across all World Cup matches on a heatmap. A heatmap, in this case, is good to show intensity in data through the use of a blue colour gradient which signifies the surplus of points earned throughout the World Cups. From there, we can see that South America and Europe usually dominate the World Cups, with Germany and Brazil scoring most of the points.
...
ggplot(top_countries_joined, aes(x = long, y = lat, group = group)) +
  ...
  scale_fill_gradientn(name = "Total Points",
                       colors = c("#89CFF0", "#5DADE2", "#3498DB", "#2E86C1", "#1F618D"),
                       na.value = "white") +
...
We want to investigate whether home country advantage is a factor in winning the World Cup. To do this, we manipulate the data to get the proportion of games a country wins when they are the host of the World Cup against when they are not the host, which are then plotted on the scatter plot. The scatter plot allows us to see the relationship between two continuous variables (in this case, the proportions of home vs away games). Moreover, it allows us to plot a regression, which in this case shows that countries that lie above regression line have higher proportion of wins at home while countries that lie below the regression line has higher proportion of wins when away. Brazil has the highest proportion of wins at home (70.83%) while Russia has the highest proportion of wins when away (60%).
...
# Plotting
ggplot(data = wc_win_prop_combine, aes(x = host_win, y= away_wins_prop))+
  geom_point()+
...
Lastly, we want to find whether or not the progress of the world cup affects whether or not a country performs well in the world cup. 
