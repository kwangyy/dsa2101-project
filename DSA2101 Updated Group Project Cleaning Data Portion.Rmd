...
``` {r}
combine_data <- wc_win_prop_combine%>%
  left_join(wc_host_wins, by =c('country'))%>%
  rename('host_win_prop' = 'host_win.x')%>%
  select(country,not_host_wins_prop, host_win_prop)


ggplot(data = combine_data, aes(x = proportion_away_win, y = host_wins))+
  geom_point()+
  geom_text(aes(label = country), hjust= 0.6,vjust = -0.5,angle = 45,size = 3)+
  ylim(0,110)+
  geom_abline(color = 'red',linetype=2) +
  labs(title = 'Host Country Performance VS when Away',
